...
{
    private static final int EDGETHRESHOLD = 20;    
        ...
        for (int row = 0; row < pi.getHeight(); row++) {
            for (int col = 0; col < pi.getWidth(); col++) {
                ...
                if(row != 0 && row != pi.getHeight()-1 && col != 0 && col != pi.getWidth()-1){
                    //right pixel
                    if(compareBrightness(data[row][col],data[row][col + 1])){
                        newData[row][col].setAllColors(0,0,0);
                    }
                    //left pixel
                    else if(compareBrightness(data[row][col],data[row][col - 1])){
                        newData[row][col].setAllColors(0,0,0);
                    }
                    //down pixel
                    else if(compareBrightness(data[row][col],data[row + 1][col])){
                        newData[row][col].setAllColors(0,0,0);
                    }
                    //up pixel
                    else if(compareBrightness(data[row][col],data[row - 1][col])){
                    ...
                    //bottom right pixel
                    else if(compareBrightness(data[row][col],data[row + 1][col + 1])){
                        newData[row][col].setAllColors(0,0,0);
                    }
                    //bottom left pixel
                    else if(compareBrightness(data[row][col],data[row + 1][col - 1])){
    ...
    public static double getAvg(Pixel pixel){
        return Math.abs(((pixel.getRed()+pixel.getBlue()+pixel.getGreen())/3));
    }

    public static boolean compareBrightness(Pixel p1, Pixel p2){
        return getAvg(p1) - getAvg(p2) > EDGETHRESHOLD;
    }
